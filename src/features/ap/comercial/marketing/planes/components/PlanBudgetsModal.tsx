...
import { BUDGETS, BUDGET_TYPE_OPTIONS } from "@/features/ap/comercial/marketing/presupuestos/lib/budgets.constants";
...
export default function PlanBudgetsModal({ plan, onOpenChange }: Props) {
  ...
  const [creatingType, setCreatingType] = useState<"regular" | "additional" | null>(null);

  const { data: budgets = [], isLoading, refetch } = useAllBudgets(
    plan ? { plan_id: plan.id } : undefined,
  );
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: (error: any) => {
      ...
      errorToast(ERROR_MESSAGE({ name: "Presupuesto", gender: true }, "create", msg));
    },
  });
    ...
    <Dialog open={!!plan} onOpenChange={(open) => !open && onOpenChange(false)}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Los presupuestos regulares y adicionales de este plan se agregan aquí.
          </DialogDescription>
        </DialogHeader>
        ...
        {creatingType ? (
          ...
          <div className="space-y-4">
            <div className="flex gap-2 flex-wrap justify-end">
              <Button size="sm" variant="outline" onClick={() => setCreatingType("regular")}>
                <Plus className="size-4 mr-2" /> Presupuesto Regular
              </Button>
              <Button size="sm" variant="outline" onClick={() => setCreatingType("additional")}>
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="font-medium capitalize">
                        {budget.type_label ??
                          ...
                          budget.type}
                        {budget.period_month ? ` · Mes ${budget.period_month}` : ""}
                      </span>
                      <span className="text-muted-foreground">
                        {budget.currency?.symbol ?? ""} {Number(budget.amount_estimated).toFixed(2)}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge className="capitalize">{budget.status_label ?? budget.status}</Badge>
