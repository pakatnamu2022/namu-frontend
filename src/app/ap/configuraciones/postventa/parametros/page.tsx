...
import {
  AFTER_SALES_PARAMETERS_TYPE,
  POST_SALE_MASTERS,
} from "@/features/gp/maestros-generales/lib/generalMasters.constants";

export default function AfterSalesParameterPage() {
  const { checkRouteExists, isLoadingModule, currentView } = useCurrentModule();
  const { ROUTE } = POST_SALE_MASTERS;
  ...
  const { data, isLoading, refetch } = useGeneralMasters({
    params: {
      ...
      search,
      per_page,
      type: AFTER_SALES_PARAMETERS_TYPE,
    ...
    <div className="space-y-4">
      ...
      {showCreate && (
        <GeneralMastersModal
          ...
          mode="create"
          defaultType={AFTER_SALES_PARAMETERS_TYPE}
          lockedType={true}
        />
      )}
