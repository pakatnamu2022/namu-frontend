...
import { PRODUCT_TALLER } from "@/features/ap/post-venta/gestion-almacen/productos/lib/product.constants";
import {
  findProductById,
  updateProduct,
...
export default function UpdateProductTallerPage() {
  const { id } = useParams();
  const router = useNavigate();
  const queryClient = useQueryClient();
  const { currentView, checkRouteExists } = useCurrentModule();
  const { ROUTE, QUERY_KEY, MODEL, ABSOLUTE_ROUTE } = PRODUCT_TALLER;
