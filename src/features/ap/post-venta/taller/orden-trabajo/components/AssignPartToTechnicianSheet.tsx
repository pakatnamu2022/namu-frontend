...
const assignSchema = z.object({
  ...
  delivered_quantity: requiredDecimalNumber("Ingrese una cantidad válida"),
});
...
export function AssignPartToTechnicianSheet({
  ...
  const form = useForm<AssignFormValues>({
    ...
    defaultValues: { delivered_to: "", delivered_quantity: "" },
  });
  ...
  const mutation = useMutation({
    mutationFn: (values: AssignFormValues) =>
      assignPartToTechnician(part!.id, {
        ...
        delivered_quantity: Number(values.delivered_quantity),
      }),
    onSuccess: () => {
  ...
  const unassignMutation = useMutation({
    mutationFn: (deliveryId: number) => unassignPartFromTechnician(deliveryId),
    onSuccess: () => {
      successToast("Asignación eliminada exitosamente");
      queryClient.invalidateQueries({
        queryKey: ["workOrderPlanning", "partsDeliveries", part?.id],
      });
      queryClient.invalidateQueries({
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {permission && (
                            <Button
                              ...
                              onClick={() =>
                                unassignMutation.mutate(delivery.id)
                              }
                            >
