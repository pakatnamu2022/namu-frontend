  ...
  try {
    ...
    const response = await api.post<{ data: TravelPhoto; message: string }>(
      `/tp/comercial/control-travel/${travelId}/photos`,
      {
        ...
        operating_system:
          deviceInfo.operatingSystem || metadata.operatingSystem,
...
export async function uploadMultiplePhotos(
  ...
  try {
    const processPhotos = photos.map((photo) => ({
      ...photo,
      photo: photo.photo.startsWith("data:")
        ? photo.photo
...
export async function getPhotos(
  ...
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Error en recuperar las fotos"
    );
  }
}
...
export async function getPhotoStats(travelId: number): Promise<PhotoStats> {
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Error al obtener estadísticas de fotos"
    );
  }
}
