...
import { PRODUCT_REPUESTOS } from "@/features/ap/post-venta/gestion-almacen/productos/lib/product.constants.ts";
...
export default function AssignWarehouseRepuestoPage() {
  const router = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { isLoadingModule, currentView } = useCurrentModule();
  const [isAssigning, setIsAssigning] = useState<number | null>(null);
  const { ABSOLUTE_ROUTE } = PRODUCT_REPUESTOS;
