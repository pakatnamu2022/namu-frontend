...
import { importHistoricalSalaries } from "../lib/payroll-calculation.actions";
...
export default function PayrollHistoricalSalaryImportDialog({
  open,
  onClose,
  companyId,
  companyName,
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    setIsLoading(true);
    try {
      const res = await importHistoricalSalaries(values.file, companyId);
      setResult(res);
  ...
  return (
    <GeneralSheet
      open={open}
      onClose={handleClose}
      title="Importar histórico de sueldos"
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Sube el Excel (.xlsx, .xls) con el histórico de sueldos — una
              fila por cada cambio de sueldo del trabajador, con la fecha
              desde la que rigió. Mismo formato que la plantilla descargable
              (columna FECHA_VIGENCIA_DESDE vacía = sueldo actual, agrega
              filas adicionales para sueldos anteriores). Encadena
              automáticamente los contratos por fecha.
            </p>
            <FormField
