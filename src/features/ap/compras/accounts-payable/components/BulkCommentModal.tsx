...
interface Props {
  open: boolean;
  selectedRecords: AccountPayable[];
  onClose: () => void;
    ...
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            El comentario se agregará a <strong>{count}</strong>{" "}
        ...
        <div className="rounded-lg border bg-muted/40 overflow-hidden">
          ...
          <div
            className={cn(
              ...
              count > 5 && "max-h-44 overflow-y-auto",
            )}
          >
            {selectedRecords.map((r) => (
              <Attachment key={r.id} size="sm" className="w-full max-w-full">
                ...
                <AttachmentContent>
                  <AttachmentTitle>{r.documento}</AttachmentTitle>
                  <AttachmentDescription>
                    {r.proveedor_nombre}
                  </AttachmentDescription>
                </AttachmentContent>
