...
import { downloadHistoricalTemplate } from "../lib/payroll-calculation.actions";
...
interface Props {
  ...
  companyId: number;
  companyName?: string;
  /** Periodo del cálculo actual, se usa solo para sugerir el primer mes de la lista */
  defaultYear: number;
  defaultMonth: number;
}

export default function PayrollHistoricalTemplateDialog({
  open,
  onClose,
  companyId,
  companyName,
  ...
  const addPeriod = () => {
    ...
    // sugiere el mes siguiente al último agregado
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadHistoricalTemplate(companyId, periods);
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Plantilla de histórico de conceptos variables</DialogTitle>
          <DialogDescription>
            {companyName ? `Empresa: ${companyName}. ` : ""}
            Genera una plantilla Excel con los trabajadores activos y una fila
            por trabajador+mes (en cero) para los meses que elijas, lista para
            completar a mano y volver a subir con "Importar Histórico".
          </DialogDescription>
        </DialogHeader>
