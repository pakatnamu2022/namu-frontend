...
import {
  ...
  TripStatus,
} from "@/features/tp/comercial/ControlViajes/lib/travelControl.interface";
import {
  changeStatus,
  endRoute,
...
export const useFilteredTravels = (filters: {
  search?: string;
...
export const useRegisterFuel = () => {
  ...
  return useMutation({
    ...
    onSuccess: (
      data: { travel: TravelControlResource; fuel: any },
      variables
    ) => {
      // Especificar tipo
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY] });
