...
export const KanbanCard = <T extends KanbanItemProps = KanbanItemProps>({
  ...
  // Filter out button clicks from drag listeners
  const isButtonClick = (e: any) => {
...
>({
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    // Solo llamar al callback sin modificar el estado interno
    // Esto permite que el componente padre controle completamente el estado
    onDragOver?.(event);
  };
  const handleDragEnd = (event: DragEndEvent) => {
    setActiveCardId(null);
    onDragEnd?.(event);
    // No modificar el estado interno aquí - dejar que el componente padre
    // controle completamente cuándo y cómo actualizar los datos
  };
  ...
  return (
    <KanbanContext.Provider value={{ columns, data, activeCardId }}>
      ...
      >
        <div
          className={cn(
            "flex gap-4 overflow-x-auto pb-4", // ✅ CAMBIAR: flex con scroll horizontal
            className
          )}
        >
