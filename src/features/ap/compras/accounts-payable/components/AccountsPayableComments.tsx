...
import {
  ...
  updateAccountPayableComment,
  deleteAccountPayableComment,
} from "../lib/accountsPayable.actions";
import type { AccountPayableComment } from "../lib/accountsPayable.interface";
            ...
            <MessageAvatar>
              <div
                className={cn(
                  ...
                  isSystem ? "bg-slate-100 text-slate-500" : avatarColor(name),
                )}
              >
...
interface Props {
  selectedId: number;
  canUpdate: boolean;
  initialComments: AccountPayableComment[];
}

export default function AccountsPayableComments({
  selectedId,
  canUpdate,
  initialComments,
}: Props) {
  ...
  const [localComments, setLocalComments] = useState<AccountPayableComment[]>(
    [],
  );
  ...
  const handleSaveComment = async () => {
    ...
    setIsSaving(true);
    try {
      const created = await addAccountPayableComment(
        selectedId,
        newComment.trim(),
      );
  ...
  const handleEditComment = async (id: number, text: string) => {
    try {
      await updateAccountPayableComment(id, text);
      setLocalComments(
  ...
  const handleDeleteComment = async (id: number) => {
    try {
      await deleteAccountPayableComment(id);
