...
const ControlTravelStatusEnum = z.enum([
  "pending",
  "in_progress",
  "completed",
  "fuel_pending",
...
export const ControlTravelSchemaCreate = ControlTravelBaseSchema.superRefine(
  (data, ctx) => {
    if (
      data.finalKm !== undefined &&
      data.initialKm !== undefined &&
      data.finalKm <= data.initialKm
    ) {
      ctx.addIssue({
...
export const ControlTravelSchemaUpdate =
  ControlTravelBaseSchema.partial().extend({
...
export const ControlTravelFuelSchema = z.object({
  ...
  fuelAmount: z.number().min(0.1, "El monto debe ser mayor a 0"),
  factorKm: z
    .number()
    .min(0.1, "El factor KM debe ser mayor a 0")
    .max(10, "El factor KM no puede exceder 10"),
  fuelDate: z.coerce.date().max(new Date(), "La fecha no puede ser futura"),
  odometerReading: z
    .number()
  ...
  return z
    .object({
      ...
      fuelAmount: z
        ...
        .optional()
        .refine(
          (val) => !val || (!isNaN(parseFloat(val)) && parseFloat(val) >= 0.1),
          {
    ...
    .superRefine((data, ctx) => {
      ...
      const initialFromTrip =
        options?.tripInitialKm ?? parseFloat(data.initialKm);

      if (
        !isNaN(final) &&
        !isNaN(initialFromTrip) &&
        final <= initialFromTrip
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
...
export type TravelControlModalData = z.infer<
  typeof TravelControlModalBaseSchema
>;
export type ControlTravelSchema = z.infer<typeof ControlTravelSchemaCreate>;
export type ControlTravelUpdateSchema = z.infer<
  typeof ControlTravelSchemaUpdate
>;
export type ControlTravelFuelSchema = z.infer<typeof ControlTravelFuelSchema>;
export type ControlTravelStatusSchema = z.infer<
  typeof ControlTravelStatusSchema
>;
